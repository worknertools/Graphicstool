(() => {
  ...
  function downloadBlob(blob, filename) {
    ...
    a.href = url;
    a.download = filename; // 关键：强制触发下载而不是跳转
  ...
  function handleExportSVG() {
    ...
    for (let i = 0; i < cachedEdges.length; i += step) {
      ...
      if (styleMode === 'square') {
        ...
        const r = (p.rectSize / 2).toFixed(2);
        shapes.push(`<circle cx="${cx}" cy="${cy}" r="${r}" fill="${foregroundColor}"/>`);
      } else if (styleMode === 'filled') {
        const cx = (p.drawX + p.rectSize / 2).toFixed(2);
        const cy = (p.drawY + p.rectSize / 2).toFixed(2);
        ...
        const innerR = Math.max(baseR - stroke, 0).toFixed(2);
        if (parseFloat(innerR) > 0.4) {
          shapes.push(`<circle cx="${cx}" cy="${cy}" r="${innerR}" fill="${foregroundColor}"/>`);
        }
      }
    }

    const svgContent = `
<svg width="${w}px" height="${h}px" viewBox="0 0 ${w} ${h}" xmlns="http://www.w3.org/2000/svg" style="background:${backgroundColor}">
  ${shapes.join('\n  ')}
</svg>`.trim();
